...
.controller('RecorderCtrl', function($scope, $stateParams, $http,$ionicPopup, $rootScope,$ionicSideMenuDelegate){
	...
	$rootScope.startRecorder = function(){
		var settings = {
			...
			"url": "http://"+$rootScope.data.serverIP+"/api/recorder",
			"method": "PUT",
			"data": JSON.stringify({recording:true, streaming:true})
		}
		$http(settings).then(function(response) {
			console.log(response);
...
.controller('VideoServerCtrl',function ($rootScope, $scope, $sce,$ionicSideMenuDelegate, $location,$ionicViewSwitcher, $state, $timeout, $http, $ionicPopup, $interval) {
	...
	this.config = {
		sources: [
			{src: $sce.trustAsResourceUrl("http://84.104.56.233:8080/video/live.m3u8"), type: "application/x-mpegURL"}
	...
	$scope.onSwipeUp = function(){
		...
		$timeout(function() { $state.go('app.events'); }); 
	}
		$rootScope.checkStatus = function(){
		var settings = {
			"async": true,
...
.controller('StreamCtrl',function ($rootScope, $scope, $sce,$ionicSideMenuDelegate, $location,$ionicViewSwitcher, $state, $timeout, $http, $ionicPopup, $interval) {
